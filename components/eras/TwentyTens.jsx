    ...
    >
      <div
        style={{
          ...
          background: 'linear-gradient(to right,#FF3B30,#FF9500,#FFCC00,#34C759,#007AFF,#5856D6,#FF2D55)',
          zIndex: 20, pointerEvents: 'none',
        }}
      />

      <div className="era-watermark" style={{ fontFamily: 'var(--font-outfit)', color: '#1a1a1a', fontWeight: 900 }}>
        2010s
      </div>
      ...
      <div className="relative h-full grid" style={{ gridTemplateColumns: '1fr 1fr', zIndex: 10 }}>
        ...
        >
          ...
          <div
            ref={statsRef}
            style={{ display: 'flex', gap: 'clamp(20px,3.5vw,40px)', marginBottom: '32px' }}
          >
            {STATS.map((s) => (
              <div key={s.value}>
                <div style={{ fontFamily: 'var(--font-outfit)', fontWeight: '800', fontSize: 'clamp(20px,2.5vw,32px)', color: '#1a1a1a', lineHeight: 1 }}>
                  {s.value}
                </div>
                <div style={{ fontFamily: 'var(--font-outfit)', fontSize: 'clamp(9px,0.9vw,11px)', color: '#999', marginTop: '3px', textTransform: 'uppercase', letterSpacing: '0.12em' }}>
                  {s.label}
                </div>
              </div>
            ))}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 'clamp(8px,1.5vw,18px)', maxWidth: 'clamp(180px,22vw,260px)' }}>
            {APPS.map((app, i) => (
              <div
                key={app.name}
                ref={(el) => (appIconRefs.current[i] = el)}
              ...
              >
                ...
                <span style={{ fontFamily: 'var(--font-outfit)', fontSize: 'clamp(7px,0.7vw,9px)', color: '#666', textAlign: 'center', fontWeight: '500' }}>
                  {app.name}
                </span>
              </div>
            ))}
        ...
        <div
          ...
          style={{ padding: 'clamp(16px,3vw,48px) clamp(16px,3.5vw,48px) clamp(16px,3vw,48px) clamp(12px,2.5vw,32px)' }}
        >
              ...
              >
                ...
                <div style={{ padding: '9px 12px', flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '2px', flexWrap: 'wrap' }}>
                    <span style={{ fontFamily: 'var(--font-outfit)', fontWeight: '800', fontSize: 'clamp(12px,1.4vw,16px)', color: m.color }}>
                      {m.year}
                    </span>
                    <span style={{ fontFamily: 'var(--font-outfit)', fontWeight: '700', fontSize: 'clamp(8px,0.85vw,11px)', color: '#1a1a1a', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                      {m.event}
                    </span>
                  </div>
                  <div style={{ fontFamily: 'var(--font-outfit)', fontSize: 'clamp(9px,0.95vw,12px)', color: '#777', lineHeight: 1.45 }}>
                    {m.desc}
                  </div>
                </div>
              </div>
