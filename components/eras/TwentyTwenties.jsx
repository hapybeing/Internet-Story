...
export default function TwentyTwenties() {
  ...
  return (
    ...
    >
      ...
      <div className="era-watermark" style={{ fontFamily: 'var(--font-outfit)', color: '#a78bfa', fontWeight: 900 }}>
        2020s
      </div>
      ...
      <div className="relative h-full grid" style={{ gridTemplateColumns: '1fr 1fr', zIndex: 10, alignItems: 'center' }}>
        ...
        <div className="flex flex-col justify-center" style={{ padding: 'clamp(24px,5vw,64px) clamp(16px,4vw,48px)' }}>
          ...
          <div
            ref={subtitleRef}
            style={{ fontFamily: 'var(--font-outfit)', fontSize: 'clamp(9px,1vw,12px)', letterSpacing: '0.35em', color: 'rgba(167,139,250,0.6)', marginBottom: '16px', textTransform: 'uppercase', fontWeight: '600' }}
          >
            2020 — 2025
          </div>
          ...
          <h1
            ref={titleRef}
            style={{ fontFamily: 'var(--font-outfit)', fontWeight: '900', fontSize: 'clamp(28px,4.8vw,62px)', color: '#fff', lineHeight: 1.05, marginBottom: '22px', letterSpacing: '-0.02em' }}
          >
          ...
          <div ref={chatRef} className="glass-card" style={{ maxWidth: 'clamp(280px,35vw,380px)', overflow: 'hidden' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: 'linear-gradient(135deg,#a78bfa,#6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', flexShrink: 0 }}>
                ✦
              </div>
            ...
            <div style={{ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: '7px' }}>
              {CHAT_MESSAGES.map((msg, i) => (
                <div
                  key={i}
                  ref={(el) => { chatMsgRefs.current[i] = el; if (el) el.dataset.role = msg.role }}
                  style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}
                >
                  <div
                    style={{
                      background: msg.role === 'user' ? 'linear-gradient(135deg,#7c3aed,#6366f1)' : 'rgba(255,255,255,0.07)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {MILESTONES.map((m, i) => (
              <div
                ...
                style={{ display: 'flex', overflow: 'hidden', padding: 0, borderRadius: '10px' }}
              >
